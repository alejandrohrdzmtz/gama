...
```bash
# Install
go install github.com/your-org/gama@latest
...
```mermaid
flowchart TD
    LB[Load Balancer] --> PA[gama Pod A\nZone A]
    LB --> PB[gama Pod B\nZone B]
    LB --> PC[gama Pod C\nZone C]
...
import (
    "context"
    "github.com/your-org/gama/plugin"
...
spec:
  ...
  template:
    spec:
      containers:
      - name: gama
        image: ghcr.io/your-org/gama:latest
...
```bash
git clone https://github.com/your-org/gama
cd gama
